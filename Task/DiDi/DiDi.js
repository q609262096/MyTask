...
!(async () => {
  $cmp.TokenValue = decodeURIComponent(process.env.DIDI_TOKEN);
  $cmp.CityValue = process.env.DIDI_CITY;
  await Checkin()
})().finally(() => $cmp.done())
...
function Checkin () {
  return new Promise((resolve, reject) => {
    let subTitle = ''
    let detail = ''
    let CheckinURL = mainURL + 'city_id=' + $cmp.CityValue
    if (aff) {
